...
*
*  \copyright  (c) 2019 NETINT Technologies Inc. All rights reserved.
*******************************************************************************/
...
ni_retcode_t ni_decoder_session_open(ni_session_context_t* p_ctx)
{
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ...
    {
      p_ctx->pkt_user_data_unreg_sei[i] = NULL;
      p_ctx->pkt_user_data_unreg_sei_len[i] = 0;
    }
    p_ctx->last_pkt_user_data_unreg_sei = NULL;
    p_ctx->last_pkt_user_data_unreg_sei_len = 0;
    ...
    p_ctx->codec_start_time = tv.tv_sec*1000000 + tv.tv_usec;
#if NI_DEBUG_LATENCY
...
ni_retcode_t ni_decoder_session_close(ni_session_context_t* p_ctx, int eos_recieved)
{
  ...
  for (i = 0; i < NI_FIFO_SZ; i++)
  {
    free(p_ctx->pkt_user_data_unreg_sei[i]);
    p_ctx->pkt_user_data_unreg_sei[i] = NULL;
    p_ctx->pkt_user_data_unreg_sei_len[i] = 0;
  }

  free(p_ctx->last_pkt_user_data_unreg_sei);
  p_ctx->last_pkt_user_data_unreg_sei = NULL;
  p_ctx->last_pkt_user_data_unreg_sei_len = 0;

  struct timeval tv;
  gettimeofday(&tv, NULL);
  uint64_t codec_end_time = tv.tv_sec*1000000 + tv.tv_usec;
...
int ni_decoder_session_write(ni_session_context_t* p_ctx, ni_packet_t* p_packet)
{
  ...
  if ((p_packet->dts != NI_NOPTS_VALUE) && (p_ctx->frame_time_q != NULL)) {
      ...
      ni_lat_meas_q_add_entry(p_ctx->frame_time_q, (logtv.tv_sec*1000000000LL+logtv.tv_nsec), p_packet->dts);
  }
#endif
  ...
  else
  {
    ...
    /* support user data unregistered SEI software passthough in 512 aligned mode only. */
    /* if this wrap-around pkt_offset_index spot is about to be overwritten, free the previous one. */
    free(p_ctx->pkt_user_data_unreg_sei[p_ctx->pkt_index % NI_FIFO_SZ]);

    if (p_packet->p_user_data_unreg_sei)
    {
      p_ctx->pkt_user_data_unreg_sei[p_ctx->pkt_index % NI_FIFO_SZ] = malloc(p_packet->user_data_unreg_sei_len);
      if (p_ctx->pkt_user_data_unreg_sei[p_ctx->pkt_index % NI_FIFO_SZ])
      {
        memcpy(p_ctx->pkt_user_data_unreg_sei[p_ctx->pkt_index % NI_FIFO_SZ], p_packet->p_user_data_unreg_sei,
               p_packet->user_data_unreg_sei_len);
        p_ctx->pkt_user_data_unreg_sei_len[p_ctx->pkt_index % NI_FIFO_SZ] = p_packet->user_data_unreg_sei_len;
      }
      else
      {
        /* warn and lose the sei data. */
        ni_log(NI_LOG_ERROR, "%s: failed to allocate user data unregistered SEI buffer for pkt.\n",
               __func__);
      }

      if (p_packet->no_slice)
      {
        /* this pkt contains udu sei data, but no slice data.
         * store it for the next pkt which contains slice data. */
        p_ctx->last_pkt_user_data_unreg_sei = malloc(p_packet->user_data_unreg_sei_len);
        if (p_ctx->last_pkt_user_data_unreg_sei)
        {
          memcpy(p_ctx->last_pkt_user_data_unreg_sei, p_packet->p_user_data_unreg_sei, p_packet->user_data_unreg_sei_len);
          p_ctx->last_pkt_user_data_unreg_sei_len = p_packet->user_data_unreg_sei_len;
        }
        else
        {
          /* warn and lose the sei data. */
    ...
    else if (p_ctx->last_pkt_user_data_unreg_sei)
    {
      ...
      p_ctx->pkt_user_data_unreg_sei[p_ctx->pkt_index % NI_FIFO_SZ] = p_ctx->last_pkt_user_data_unreg_sei;
      p_ctx->pkt_user_data_unreg_sei_len[p_ctx->pkt_index % NI_FIFO_SZ] = p_ctx->last_pkt_user_data_unreg_sei_len;
      p_ctx->last_pkt_user_data_unreg_sei = NULL;
      p_ctx->last_pkt_user_data_unreg_sei_len = 0;
    }
    else
    {
      p_ctx->pkt_user_data_unreg_sei[p_ctx->pkt_index % NI_FIFO_SZ] = NULL;
      p_ctx->pkt_user_data_unreg_sei_len[p_ctx->pkt_index % NI_FIFO_SZ] = 0;
    }
...
int ni_decoder_session_read(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ...
  unsigned int bytes_read_so_far = 0;
    ...
    if (inst_info.rd_buf_avail_size < totall_bytes_to_read)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_read() avaliable size(%u)"
             "less than needed (%u)\n",
             inst_info.rd_buf_avail_size, totall_bytes_to_read);
      ni_assert(0);
    }
    ...
    else
    {
      ...
      else
      {
        ...
        if (frame_offset == 0)
        {
          ...
          if (p_ctx->pkt_user_data_unreg_sei[0])
          {
            p_frame->p_user_data_unreg_sei = p_ctx->pkt_user_data_unreg_sei[0];
            p_frame->user_data_unreg_sei_len = p_ctx->pkt_user_data_unreg_sei_len[0];
            p_ctx->pkt_user_data_unreg_sei[0] = NULL;
            p_ctx->pkt_user_data_unreg_sei_len[0] = 0;
          }
        }
        else
        {
          for (i = 0 ; i < NI_FIFO_SZ ; i++)
          {
                ...
                &&(frame_offset  < p_ctx->pkt_offsets_index[i]))
            {
              ...
              if (p_ctx->pkt_user_data_unreg_sei[i % NI_FIFO_SZ])
              {
                p_frame->p_user_data_unreg_sei = p_ctx->pkt_user_data_unreg_sei[i % NI_FIFO_SZ];
                p_frame->user_data_unreg_sei_len = p_ctx->pkt_user_data_unreg_sei_len[i % NI_FIFO_SZ];
                p_ctx->pkt_user_data_unreg_sei[i % NI_FIFO_SZ] = NULL;
                p_ctx->pkt_user_data_unreg_sei_len[i % NI_FIFO_SZ] = 0;
              }
              break;
            }
            if (i == (NI_FIFO_SZ-1))
...
ni_retcode_t ni_encoder_session_open(ni_session_context_t* p_ctx)
{
  ...
  uint32_t buffer_size = 0;
  void *p_signature = NULL;
  ni_encoder_config_t* p_cfg = (ni_encoder_config_t*)p_ctx->p_session_config;
  uint32_t model_load = 0;
  ni_instance_status_info_t inst_info = { 0 };
  void* p_buffer = NULL;
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ...
    memset(p_buffer, 0, NI_DATA_BUFFER_LEN);
    session_info.codec_format = ni_htonl(p_ctx->codec_format);
    session_info.i32picWidth = ni_htonl(p_cfg->i32picWidth);
    session_info.i32picHeight = ni_htonl(p_cfg->i32picHeight);
    session_info.model_load = ni_htonl(model_load);
    ...
    CREATE_SESSION_SET_DW12_ENC_CID_FRWIDTH(cmd.cdw12, p_ctx->codec_format, p_cfg->i32picWidth);
    CREATE_SESSION_SET_DW13_ENC_FRHIGHT(cmd.cdw13, p_cfg->i32picHeight);
    CREATE_SESSION_SET_DW14_MODEL_LOAD(cmd.cdw14, model_load);
...
int ni_encoder_session_seperate_yuv_bypass_write(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ni_nvme_result_t nvme_result[NI_FW_NUM_OF_MEM_SEGS][WORKER_QUEUE_DEPTH];
  ni_nvme_command_t cmd[NI_FW_NUM_OF_MEM_SEGS][WORKER_QUEUE_DEPTH];
  ...
  ni_log(NI_LOG_TRACE,"ni_encoder_session_seperate_yuv_bypass_write(): enter\n");

  ni_log(NI_LOG_TRACE,"YUV copy bypass %d\n", p_ctx->yuv_copy_bypass);
  if ( (!p_ctx) || (!p_frame) )
  {
    ni_log(NI_LOG_TRACE,"ERROR: passed parameters are null!, return\n");
    retval = NI_RETCODE_INVALID_PARAM;
  ...
  /*!********************************************************************/
  ni_log(NI_LOG_TRACE,"ER:data_len:%d %d %d %d\n",p_frame->data_len[0], p_frame->data_len[1], p_frame->data_len[2], p_frame->extra_data_len);
  ...
  while (1)
  {
    ...
    {
      ni_log(NI_LOG_TRACE,"Warning enc write query fail rc %d or available "
  ...
  if (1 == p_ctx->force_frame_type || p_frame->force_key_frame)
  {
    ...
    ni_log(NI_LOG_TRACE,"ni_encoder_session_seperate_yuv_bypass_write(): ctx->force_frame_type"
  ...
  ni_log(NI_LOG_TRACE,"ni_encoder_session_seperate_yuv_bypass_write(): %d.%d p_ctx->frame_num=%lu, p_frame->start_of_stream=%u, p_frame->end_of_stream=%u, p_frame->video_width=%u, p_frame->video_height=%u, pts=0x%08x 0x%08x, dts=0x%08x 0x%08x, sei_len=%u, roi size=%u avg_qp=%u reconf_len=%u force_pic_qp=%u\n",
                 p_ctx->hw_id, p_ctx->session_id, p_ctx->frame_num, p_frame->start_of_stream, p_frame->end_of_stream, p_frame->video_width, p_frame->video_height, (uint32_t)((p_frame->pts >> 32) & 0xFFFFFFFF),
                 (uint32_t)(p_frame->pts & 0xFFFFFFFF), (uint32_t)((p_frame->dts >> 32) & 0xFFFFFFFF), (uint32_t)(p_frame->dts & 0xFFFFFFFF),
                 p_meta->frame_sei_data_size, p_meta->frame_roi_map_size,
                 p_meta->frame_roi_avg_qp, p_meta->enc_reconfig_data_size,
                 p_meta->force_pic_qp_i);
  ni_log(NI_LOG_TRACE,"ni_encoder_session_seperate_yuv_bypass_write p_metaadd:%p \n", p_meta);
  ...
  else //handle regular frame sending
  {
    ...
    {
#ifdef XCODER_IO_RW_ENABLED
      ni_log(NI_LOG_ERROR,"ni_encoder_session_seperate_yuv_bypass_write\n");
#else
      uint8_t* p_data = p_frame->p_buffer;
      int meta_sent = 0;
      uint32_t frame_chunk_index[NI_FW_NUM_OF_MEM_SEGS] = {0};

      //Number of chunks for YUV, 0 is Y, 1 is U, 2 is V 3 is metadatahead and meta data, if they are not io size alined, we need to add one
      uint32_t total_num_of_chunks[NI_FW_NUM_OF_MEM_SEGS] = {0};
      int idx = 0;
      for (idx = 0; idx < NI_FW_NUM_OF_MEM_SEGS-1; idx++)
      {
        total_num_of_chunks[idx] = p_frame->data_len[idx]/(p_ctx->max_nvme_io_size) + (p_frame->data_len[idx] % (p_ctx->max_nvme_io_size)? 1:0);
      }
      total_num_of_chunks[NI_META_ELEMENT] = p_frame->extra_data_len/(p_ctx->max_nvme_io_size) + (p_frame->extra_data_len % (p_ctx->max_nvme_io_size)? 1:0);

      uint32_t frame_chunk_counter[NI_FW_NUM_OF_MEM_SEGS] = {0}; // current index of the frame that is being transfered
      ni_log(NI_LOG_TRACE,"Total num of chunks Y:%d U:%d V:%d meta:%d, fsb %d, pm %d\n", \
                    total_num_of_chunks[NI_Y_ELEMENT], \
                    total_num_of_chunks[NI_U_ELEMENT], \
                    total_num_of_chunks[NI_V_ELEMENT], \
                    total_num_of_chunks[NI_META_ELEMENT], \
                    frame_size_bytes, \
                    p_ctx->max_nvme_io_size);
 
      ni_log(NI_LOG_TRACE,"WARN: p_data = %p, p_frame->buffer_size = %u frame_chunk_index Y:%d U:%d V:%d meta:%d\n", \
                      p_data, \
                      p_frame->buffer_size, \
                      frame_chunk_index[NI_Y_ELEMENT], \
                      frame_chunk_index[NI_U_ELEMENT], \
                      frame_chunk_index[NI_V_ELEMENT], \
                      frame_chunk_index[NI_META_ELEMENT]
                      );
      ...
      if (NI_RETCODE_SUCCESS != retval)
      {
        ni_log(NI_LOG_TRACE,"ERROR: ni_encoder_session_seperate_yuv_bypass_write(): ni_timestamp_register() for dts returned: %d\n", retval);
      }

      // Now the metadata chunks will be sent down first,  no matter how many chunks there are, it needs to be sent down first and we wait for it to finish

      // Then we will queue the Y, U, V chunks in the queue. Although we queue it in sequence, the actual nvme write command may out of order. Each chunk
      // contians the offset for it's YUV and is in DW14. If the YUV format does not change from the FW side we can cauculate this from the FW side based on the
      // resolution, however the YUV format may change later, so at this point host side caculate it and tell FW where to write
      
      int32_t frame_remain_bytes = frame_size_bytes;
      int32_t frame_remain_bytes_meta = p_frame->extra_data_len;
      int32_t meta_first_chunk_size = 0;
      // total number of chunks
      int num_of_thread_to_release = total_num_of_chunks[NI_Y_ELEMENT] + total_num_of_chunks[NI_U_ELEMENT] + total_num_of_chunks[NI_V_ELEMENT] + total_num_of_chunks[NI_META_ELEMENT];
 
      // This is for the meta data only!
      for (; frame_chunk_counter[NI_META_ELEMENT] < total_num_of_chunks[NI_META_ELEMENT]; frame_chunk_counter[NI_META_ELEMENT]++)
      {
          ni_log(NI_LOG_TRACE,"Send down the meta, meta frame chunk counter %d  opcode %d remain %d send length %d\n" ,\
                          frame_chunk_counter[NI_META_ELEMENT], \
                          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].opcode, \
                          frame_remain_bytes_meta, \
                          ((frame_remain_bytes_meta) < (p_ctx->max_nvme_io_size)) ? frame_remain_bytes_meta :((p_ctx->max_nvme_io_size)));
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].opcode = nvme_cmd_xcoder_write;
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].handle = p_ctx->blk_io_handle;
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].p_ni_nvme_cmd = &(cmd[NI_META_ELEMENT][frame_chunk_counter[NI_META_ELEMENT]]);
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].data_len = ((frame_remain_bytes_meta) < (p_ctx->max_nvme_io_size)) ? frame_remain_bytes_meta :((p_ctx->max_nvme_io_size));
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].p_data = p_data + p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT] + p_frame->data_len[NI_V_ELEMENT];
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].p_result = &(nvme_result[NI_META_ELEMENT][frame_chunk_counter[NI_META_ELEMENT]]);
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].frame_chunk_index = ((frame_chunk_counter[NI_META_ELEMENT] * p_ctx->max_nvme_io_size)/FRAME_CHUNK_INDEX_SIZE); // address of set that is 4K based
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].frame_chunk_size = p_ctx->max_nvme_io_size;
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].dw14_yuv_offset = p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT] + p_frame->data_len[NI_V_ELEMENT];
          p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].session_id = p_ctx->session_id;
          frame_remain_bytes_meta = (frame_remain_bytes_meta - p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].data_len);
          ...
          // We still have some space in the queue so enque the argument
          ni_log(NI_LOG_TRACE,"Put down the lock\n");
          pthread_mutex_lock((p_ctx->encoder_write_mutex));
          worker_q_enq(&(p_ctx->encoder_write_workerqueue), p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]]);
          pthread_mutex_unlock((p_ctx->encoder_write_mutex));
          
          // First 16 bytes of the meta needs to go first, so there we need to send the header chunk of metadata
          if (frame_chunk_counter[NI_META_ELEMENT] == 0)
          {
            p_ctx->encoder_write_total_num_of_bytes = p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].data_len;
            sem_post(p_ctx->encoder_write_semaphore);
            num_of_thread_to_release--;
            pthread_mutex_lock((p_ctx->encoder_write_mutex_len));
            while (p_ctx->encoder_write_processed_data_len != p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].data_len)
            {
             pthread_cond_wait((p_ctx->encoder_write_cond),(p_ctx->encoder_write_mutex_len));
            }
            meta_first_chunk_size = p_ctx->encoder_write_processed_data_len;
            pthread_mutex_unlock((p_ctx->encoder_write_mutex_len));
            p_ctx->encoder_write_total_num_of_bytes = p_frame->extra_data_len - p_ctx->args_encoder_write[frame_chunk_counter[NI_META_ELEMENT]].data_len;
          }
          
      }
      ni_log(NI_LOG_TRACE,"Metadata sent already\n");


      // This is for YUV buffer chunks, need to calculate the chunks seperately for Y U and V,

      // We use the Zero copy pointer from here
      p_data = p_frame->p_buffer_yuv_bypass_y;

      int32_t YUV_chunk_remain_bytes = 0;
      int yuv_index = 0;
      for (yuv_index = 0; yuv_index < (NI_FW_NUM_OF_MEM_SEGS - 1); yuv_index++)
      {
        ni_log(NI_LOG_TRACE,"Current yuv_index:%d\n", yuv_index);
        YUV_chunk_remain_bytes = p_frame->data_len[yuv_index];
        for (; frame_chunk_counter[yuv_index] < total_num_of_chunks[yuv_index]; frame_chunk_counter[yuv_index]++)
        {
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].opcode = nvme_cmd_xcoder_write;
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].handle = p_ctx->blk_io_handle;
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].p_ni_nvme_cmd = &(cmd[yuv_index][frame_chunk_counter[yuv_index]]);
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].data_len = ((YUV_chunk_remain_bytes) < (p_ctx->max_nvme_io_size)) ? YUV_chunk_remain_bytes :((p_ctx->max_nvme_io_size));
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].p_data = p_data;
          ni_log(NI_LOG_TRACE,"yuv_index is %d\n", yuv_index);
          switch (yuv_index)
          {
            case NI_Y_ELEMENT:
              ni_log(NI_LOG_TRACE,"Set offset NI_Y_ELEMENT\n");
              p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].dw14_yuv_offset = 0;
              break;
            case NI_U_ELEMENT:
              ni_log(NI_LOG_TRACE,"Set offset NI_U_ELEMENT %d\n", p_frame->data_len[NI_Y_ELEMENT]);
              p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].dw14_yuv_offset = p_frame->data_len[NI_Y_ELEMENT];
              p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].p_data = p_frame->p_buffer_yuv_bypass_u;
              break;
            case NI_V_ELEMENT:
              ni_log(NI_LOG_TRACE,"Set offset NI_V_ELEMENT %d\n", p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT]);
              p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].dw14_yuv_offset = p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT];
              p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].p_data = p_frame->p_buffer_yuv_bypass_v;
              break;
            default:
             ni_log(NI_LOG_TRACE,"Error: Shoule not hit here, Assert\n");
             assert(false);

          }
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].p_result = &(nvme_result[yuv_index][frame_chunk_counter[yuv_index]]);
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].frame_chunk_index = ((frame_chunk_counter[yuv_index]*p_ctx->max_nvme_io_size)/FRAME_CHUNK_INDEX_SIZE); // address of set that is 4K based
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].frame_chunk_size = p_ctx->max_nvme_io_size;
          p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].session_id = p_ctx->session_id;

          ni_log(NI_LOG_TRACE,"Seperate send down the frame data, index:%d chunk counter %d remain %d data_len %d p_data:%p dw14_yuv_offset:%d\n", \
                                                                                                            yuv_index, \
                                                                                                            frame_chunk_counter[yuv_index], \
                                                                                                            YUV_chunk_remain_bytes, \
                                                                                                            ((YUV_chunk_remain_bytes) < (p_ctx->max_nvme_io_size)) ? YUV_chunk_remain_bytes :((p_ctx->max_nvme_io_size)), \
                                                                                                            p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].p_data, \
                                                                                                            p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].dw14_yuv_offset);
          YUV_chunk_remain_bytes = (YUV_chunk_remain_bytes - p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]].data_len);
          ...
          {
            pthread_mutex_lock((p_ctx->encoder_write_mutex));
            worker_q_enq(&(p_ctx->encoder_write_workerqueue), p_ctx->args_encoder_write[frame_chunk_counter[yuv_index]]);
            pthread_mutex_unlock((p_ctx->encoder_write_mutex));
          }
        }
      ...
      ni_log(NI_LOG_TRACE,"Now release the workers num_of_thread_to_release:%d encoder_write_total_num_of_bytes:%d total size without meta:%d\n", num_of_thread_to_release, p_ctx->encoder_write_total_num_of_bytes, (frame_size_bytes - meta_first_chunk_size));
      ...
      {
        ni_log(NI_LOG_TRACE,"encoder_write_processed_data_len %d total %d\n", p_ctx->encoder_write_processed_data_len, (frame_size_bytes - meta_first_chunk_size));
        pthread_cond_wait((p_ctx->encoder_write_cond),(p_ctx->encoder_write_mutex_len));
      }
      pthread_mutex_unlock((p_ctx->encoder_write_mutex_len));
      ni_log(NI_LOG_TRACE,"all thread finished for YUV transfer frame num:%lu\n", p_ctx->frame_num);

      ni_log(NI_LOG_TRACE,"ni_encoder_session_seperate_yuv_bypass_write: sent data retries=%u, retval=%d\n", send_count, retval);
  ...
  ni_log(NI_LOG_TRACE,"ni_encoder_session_seperate_yuv_bypass_write(): exit\n");

  return retval;
}

//This one is for zero copy where the source addres is continues
int ni_encoder_session_yuv_bypass_write(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ni_nvme_result_t nvme_result[NI_FW_NUM_OF_MEM_SEGS][WORKER_QUEUE_DEPTH];
  ni_nvme_command_t cmd[NI_FW_NUM_OF_MEM_SEGS][WORKER_QUEUE_DEPTH];
  ...
  if (1 == p_ctx->force_frame_type || p_frame->force_key_frame)
  {
    ...
    ni_log(NI_LOG_TRACE,"ni_encoder_session_yuv_bypass_write(): ctx->force_frame_type"
                   " %d frame->force_key_frame %d force frame_num %lu"
                   " type to %d\n", 
                   p_ctx->force_frame_type, p_frame->force_key_frame,
                   p_ctx->frame_num, p_frame->ni_pict_type);
  ...
  ni_log(NI_LOG_TRACE,"ni_encoder_session_yuv_bypass_write(): %d.%d p_ctx->frame_num=%lu, p_frame->start_of_stream=%u, p_frame->end_of_stream=%u, p_frame->video_width=%u, p_frame->video_height=%u, pts=0x%08x 0x%08x, dts=0x%08x 0x%08x, sei_len=%u, roi size=%u avg_qp=%u reconf_len=%u force_pic_qp=%u\n",
                 p_ctx->hw_id, p_ctx->session_id, p_ctx->frame_num, p_frame->start_of_stream, p_frame->end_of_stream, p_frame->video_width, p_frame->video_height, (uint32_t)((p_frame->pts >> 32) & 0xFFFFFFFF),
                 (uint32_t)(p_frame->pts & 0xFFFFFFFF), (uint32_t)((p_frame->dts >> 32) & 0xFFFFFFFF), (uint32_t)(p_frame->dts & 0xFFFFFFFF),
                 p_meta->frame_sei_data_size, p_meta->frame_roi_map_size,
                 p_meta->frame_roi_avg_qp, p_meta->enc_reconfig_data_size,
                 p_meta->force_pic_qp_i);
  ni_log(NI_LOG_TRACE,"ni_encoder_session_yuv_bypass_write p_metaadd:%p \n", p_meta);
  ...
  else //handle regular frame sending
  {
    ...
    {
#ifdef XCODER_IO_RW_ENABLED
      ni_log(NI_LOG_ERROR,"ni_encoder_session_yuv_bypass_write\n");
#else
      uint8_t* p_data = p_frame->p_buffer;
      int meta_sent = 0;
      uint32_t frame_chunk_index[NI_FW_NUM_OF_MEM_SEGS] = {0};

      //Number of chunks for YUV, 0 is Y, 1 is U, 2 is V 3 is metadatahead and meta data, in this case only Y can metadata value are needed as the yuv is continues
      uint32_t total_num_of_chunks[NI_FW_NUM_OF_MEM_SEGS] = {0};
      int idx = 0;
      total_num_of_chunks[NI_Y_ELEMENT] = (p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT] + p_frame->data_len[NI_V_ELEMENT])/(p_ctx->max_nvme_io_size) + ((p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT] + p_frame->data_len[NI_V_ELEMENT]) % (p_ctx->max_nvme_io_size)? 1:0);

      total_num_of_chunks[NI_META_ELEMENT] = p_frame->extra_data_len/(p_ctx->max_nvme_io_size) + (p_frame->extra_data_len % (p_ctx->max_nvme_io_size)? 1:0);

      uint32_t frame_chunk_counter[NI_FW_NUM_OF_MEM_SEGS] = {0}; // current index of the frame that is being transfered, in this case only Y and meta are valid
      ...
      if (NI_RETCODE_SUCCESS != retval)
      {
        ni_log(NI_LOG_TRACE,"ERROR: %s(): ni_timestamp_register() for dts returned: %d\n", __FUNCTION__ , retval);
      }
      ...
      // This is for YUV buffer chunks, need to calculate the chunks seperately for Y U and V,
      // We use the zero copy pointer from here, in this case, YUV are continues so we segment based on the max io size
      p_data = p_frame->p_buffer_yuv_bypass_y;

      int32_t YUV_chunk_remain_bytes = 0;
      
      YUV_chunk_remain_bytes = p_frame->data_len[NI_Y_ELEMENT] + p_frame->data_len[NI_U_ELEMENT] + p_frame->data_len[NI_V_ELEMENT];
      for (; frame_chunk_counter[NI_Y_ELEMENT] < total_num_of_chunks[NI_Y_ELEMENT]; frame_chunk_counter[NI_Y_ELEMENT]++)
      {
        ni_log(NI_LOG_TRACE,"Send down the remaining frame data, chunk counter %d remain %d data_len %d\n", frame_chunk_counter[NI_Y_ELEMENT], \
                                                                                                          YUV_chunk_remain_bytes, \
                                                                                                          ((YUV_chunk_remain_bytes) < (p_ctx->max_nvme_io_size)) ? YUV_chunk_remain_bytes :((p_ctx->max_nvme_io_size)));
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].opcode = nvme_cmd_xcoder_write;
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].handle = p_ctx->blk_io_handle;
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].p_ni_nvme_cmd = &(cmd[NI_Y_ELEMENT][frame_chunk_counter[NI_Y_ELEMENT]]);
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].data_len = ((YUV_chunk_remain_bytes) < (p_ctx->max_nvme_io_size)) ? YUV_chunk_remain_bytes :((p_ctx->max_nvme_io_size));
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].p_data = p_data;
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].dw14_yuv_offset = 0;
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].p_result = &(nvme_result[NI_Y_ELEMENT][frame_chunk_counter[NI_Y_ELEMENT]]);
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].frame_chunk_index = ((frame_chunk_counter[NI_Y_ELEMENT]*p_ctx->max_nvme_io_size)/FRAME_CHUNK_INDEX_SIZE); // address of set that is 4K based
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].frame_chunk_size = p_ctx->max_nvme_io_size;
        p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].session_id = p_ctx->session_id;
        YUV_chunk_remain_bytes = (YUV_chunk_remain_bytes - p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]].data_len);
        ...
        {
          pthread_mutex_lock((p_ctx->encoder_write_mutex));
          worker_q_enq(&(p_ctx->encoder_write_workerqueue), p_ctx->args_encoder_write[frame_chunk_counter[NI_Y_ELEMENT]]);
          pthread_mutex_unlock((p_ctx->encoder_write_mutex));
        }
      }
...
int ni_encoder_session_write(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ...
  if ((p_frame->dts != NI_NOPTS_VALUE) && (p_ctx->frame_time_q != NULL)) {
      clock_gettime(CLOCK_REALTIME, &logtv);
      ni_lat_meas_q_add_entry(p_ctx->frame_time_q, (logtv.tv_sec*1000000000LL+logtv.tv_nsec), p_frame->dts);
  }
#endif
...
enc_read_query:
  ...
  retval = ni_nvme_send_read_cmd(p_ctx->blk_io_handle, p_ctx->event_handle,
                                 p_packet->p_data + actual_read_size_aligned,
  ...
  {
    memmove(p_packet->p_data + actual_read_size,
            p_packet->p_data + actual_read_size_aligned +
...
int ni_create_frame(ni_frame_t* p_frame, uint32_t read_length, uint64_t* p_frame_offset)
{
  ...
  *p_frame_offset = 0;
  ...
  p_frame->p_user_data_unreg_sei = NULL;
  p_frame->user_data_unreg_sei_len = 0;
  ...
  {
    ni_metadata_dec_frame_t* p_meta = (ni_metadata_dec_frame_t*)
    (p_frame->p_buffer + video_data_size);
    ...
    *p_frame_offset = p_meta->metadata_common.ui64_data.frame_offset;
    ...
    if (p_meta->sei_header && p_meta->sei_number && p_meta->sei_size)
    {
                   ...
                   &ui32CCOffset, &ui32CCSize))
      {
        ...
        ni_log(NI_LOG_TRACE, "User Data Unreg size = %u\n", ui32CCSize);
      }

      if (0 == p_frame->sei_total_len)
      {
        ni_log(NI_LOG_TRACE, "Error retrieved 0 supported SEI !");
      }
    }
  }
  ...
  ni_encoder_params_t* p_src, char* p_param_err, uint32_t max_err_len)
{
  ...
  if ( p_cfg->niParamT408.intra_qp < 0 ||
