...
typedef struct _ni_instance_status_info
{
  ...
  uint32_t reserved[2];
} ni_instance_status_info_t; // 32 bytes (has to be 8 byte aligned)
...
typedef struct _ni_encoder_config_t
{
  ...
  uint8_t  ui8Reserved[7]; /**< reserved bytes **/
 ...
 *  \return frame size
 *******************************************************************************/
int ni_create_frame(ni_frame_t* p_frame, uint32_t read_length, uint64_t* frame_offset);
 ...
 *  \return
 ******************************************************************************/
void ni_set_custom_template(ni_session_context_t* p_ctx, ni_encoder_config_t* p_cfg, ni_encoder_params_t* p_src);
 ...
 *  \return
 ******************************************************************************/
ni_retcode_t ni_check_common_params(ni_t408_config_t* p_param, ni_encoder_params_t* p_src, char* param_err, uint32_t max_err_len);
...
/*!******************************************************************************
 *  \brief  When the YUV buffer that allocated by ffmpeg has continues memory,
 *  then this function is used to write the YUV and extra data to encoder
 *
 *  \param
 *
 *  \return
...
/*!******************************************************************************
 *  \brief  When the YUV buffer that allocated by ffmepeg has seperate memory,
 *  that means the Y Cr Cb is located in different locations and address are not continues
