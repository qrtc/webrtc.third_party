...
void ni_close_event(ni_event_handle_t event_handle)
{
  ...
  {
    ni_log(NI_LOG_TRACE, "Warning ni_close_event: null parameter passed %x\n", event_handle);
    return;
  }
...
void ni_device_close(ni_device_handle_t device_handle)
{
  ...
  {
    ni_log(NI_LOG_TRACE, "ERROR ni_device_close: null parameter passed %x\n", device_handle);
    return;
  }
...
ni_retcode_t ni_device_session_open( ni_session_context_t *p_ctx, ni_device_type_t device_type )
{
  ...
  if ((handle1 == NI_INVALID_DEVICE_HANDLE) && (handle == NI_INVALID_DEVICE_HANDLE))
  {
    if (p_ctx->hw_id >=0) // User selected the encoder/ decoder number
    {
      ...
      else 
      {
        ...
        // based on fw revision, determine if it accepts 512-aligned bitstream
        // and YUV copy bypass
        p_ctx->is_dec_pkt_512_aligned = 1;
        p_ctx->yuv_copy_bypass_fw_support = 0;

        char str_fw_API_ver[3];
        int fw_API_ver;
        memcpy(str_fw_API_ver,
               &(rsrc_ctx->p_device_info->fw_rev[NI_XCODER_VER_SZ + 1 + NI_XCODER_API_FLAVOR_SZ]),
    ...
    else if ((0 == strcmp(p_ctx->dev_xcoder, BEST_MODEL_LOAD_STR)) || (0 == strcmp(p_ctx->dev_xcoder, BEST_DEVICE_INST_STR)))
    {
      ...
      for (i = 0; i < num_coders; i++)
      {
        ...
        if (fw_API_ver <= 1)
        {
          p_ctx->yuv_copy_bypass_fw_support = false;
        }
        else
        {
          p_ctx->yuv_copy_bypass_fw_support = false;
      ...
      else 
      {
        ...
        // based on fw revision, determine if it accepts 512-aligned bitstream
        p_ctx->is_dec_pkt_512_aligned = 1;
        char str_fw_API_ver[NI_XCODER_FW_API_VER_SZ] = "";
        int fw_API_ver;
        memcpy(str_fw_API_ver,
               &(dev_info.fw_rev[NI_XCODER_VER_SZ + 1 + NI_XCODER_API_FLAVOR_SZ]),
               NI_XCODER_API_VER_SZ);
        str_fw_API_ver[NI_XCODER_API_VER_SZ] = '\0';
        fw_API_ver = strtol(str_fw_API_ver, NULL, 10);
        if (fw_API_ver >= 1)
        {
          p_ctx->is_dec_pkt_512_aligned = 0;
        }
  ...
  switch (device_type)
  {
    ...
    {
      for (i = 0; i< NI_MAX_OPEN_RETRIES; i++)
      {
...
#endif
      ni_log(NI_LOG_TRACE, "yuv_copy_bypass:%d yuv_copy_bypass_seperate:%d\n", p_ctx->yuv_copy_bypass, p_ctx->yuv_copy_bypass_seperate);

      if (p_ctx->yuv_copy_bypass == true)
      {
        // Here we seperate into to 2 cases:
        // 1. the YUV is all continues: then we can use max is to chunk the whole yuv buffer
        // 2. the Y,U,V are not continues: then we need to write down the Y,U,V seperately
        if (p_ctx->yuv_copy_bypass_seperate)
        {
          retval = ni_encoder_session_seperate_yuv_bypass_write(p_ctx, &(p_data->data.frame));
        }
        else
        {
          retval = ni_encoder_session_yuv_bypass_write(p_ctx, &(p_data->data.frame));
        }
